...
/*   Updated: 2019/03/07 14:41:45 by tcollard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/shell.h"

static void	redir_to_file(t_ast *elem, t_alloc *alloc, int fd[10], int i)
{
	int		fd_redir;
	int		fd_open;
	int		dig;
...
static void	redir_stdin(t_ast *elem, t_alloc *alloc, int fd[10])
{
	int		fd_file;
	int		fd_redir;
	char	*tmp;
...
int			ft_is_redir(t_ast *elem, int fd[10], t_alloc *alloc,
			t_env **lst_env)
{
...
void		reinit_fd(int fd[10], t_alloc *alloc)
{
	int	i;
