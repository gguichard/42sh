...
/*   Updated: 2019/03/19 16:21:23 by fwerner          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		, t_str_cmd_inf *str_cmd_inf, const char *token_start)
{
	...
	if ((str_cmd_cpy.str + str_cmd_cpy.pos) != token_start
		...
		return (0);
	if (end_by_and)
		if (!add_token_to_lst(token_lst, "&", TK_LRED_OPT))
			return (0);
	return (1);
}
		...
		, int last_char_was_sep)
{
	...
	{
		if (ft_strchr(CMD_SEP_CHARS, str_cmd_inf->str[str_cmd_inf->pos])
				!= NULL)
