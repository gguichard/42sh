...
static void	print_job_pipe(t_list *tmp)
{
	...
	while (tmp)
	{
		...
		if (job->state == STOPPED)
			ft_printf("      %s(%s) %d %s\n", state
						, status_stop_str(job->status), job->pid, job->cmd);
...
static void	display_job_full(t_list *tmp, int index)
{
	...
	if (job->state == STOPPED)
		ft_printf("[%d] %c %s(%s) %d %s\n", index, current, state
					, status_stop_str(job->status), job->gpid, job->cmd);
	else if (job->state == DONE && job->status)
		ft_printf("[%d] %c %s(%d) %d %s\n", index, current, state
					, job->status, job->gpid, job->cmd);
	else
		ft_printf("[%d] %c %s %d %s\n", index, current
					, state, job->gpid, job->cmd);
	if (job->pipe)
...
int			builtin_jobs(t_ast *elem, t_alloc *alloc)
{
	...
	if (opts.error)
	{
		ft_dprintf(STDERR_FILENO, "42sh: jobs: -%c: invalid option\n"
					, opts.error);
		ft_dprintf(STDERR_FILENO, "42sh: jobs: usage: jobs [-lp]"
									" [job_id ...]\n");
		return (2);
	}
