...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/04/08 13:34:27 by fwerner           #+#    #+#             */
/*   Updated: 2019/04/08 13:34:29 by fwerner          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_error		check_dir_rights(const char *path, int rights)
{
	...
	cur_slash = cur_path;
	while (cur_slash != NULL && (cur_slash == cur_path || *cur_slash != '\0'))
	{
		if ((cur_slash = ft_strchr(cur_slash, '/')) != NULL)
		{
			old_char = cur_slash[1];
			cur_slash[1] = '\0';
		}
		...
		else if (access(cur_path, F_OK | ((cur_slash == NULL
							|| old_char == '\0') ? rights : X_OK)) == -1)
			return (del_then_ret_err(cur_path, ERRC_NONEEDEDRIGHTS));
		(cur_slash != NULL && ++cur_slash != NULL) ? *cur_slash = old_char : 0;
	}
...
t_error		check_file_rights(const char *path, int file_type, int rights)
{
	...
	else if (access(path_cpy, F_OK | rights) == -1)
